...
def read_and_get_mask(img):
    try:
        ...
        print(f"Error in read_and_get_mask: {e}")




def swap_images(img1, img2,img1_landmark,img2_landmark,count):

    # mask_of_img1 = get_face_mask(img1, img1_landmark)
    # mask_of_img2 = get_face_mask(img2, img2_landmark)
    with ThreadPoolExecutor(max_workers=2) as executor:
        img1_future = executor.submit(read_image, img1)
        img2_future = executor.submit(read_image, img2)

        img1 = img1_future.result()
        img2 = img2_future.result()
    with ThreadPoolExecutor(max_workers=2) as executor:
        mask_of_img1_future = executor.submit(get_face_mask, img1, img1_landmark)
        mask_of_img2_future = executor.submit(get_face_mask, img2, img2_landmark)

        mask_of_img1 = mask_of_img1_future.result()
        mask_of_img2 = mask_of_img2_future.result()
...
def swap(number):
    ...
    for x in range(2, 2+number):
        ...
        thread = threading.Thread(target=swap_images, args=(path_1, path_2, img1_landmark, img2_landmark, x))
